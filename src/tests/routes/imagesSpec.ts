...
const getThumbnailPath = (filename: string): string => path.join(THUMBNAIL_ASSETS_DIR, filename+".jpg");

describe("Testing images route", () => {
    it("returns the image file when called with the image name and dimensions", async done => {
        const filename = "fjord";
        const response = await request.get(`/image?filename=${filename}&height=300&width=400`);
